...
for (i in 1:nrow(record_df)){
  ...
  html_Sess = record_df$eis_url[i] %>% session()
  html_Nodes <- html_Sess %>% rvest::read_html() %>% html_nodes('a')
  
  html_Nodes %>% html_attrs()
  file_url = grep('downloadAttachment',html_Nodes %>% html_attr('href') ,value=T)
  ...
  file_names = gsub('pdf\\.pdf$','.pdf',file_names)
...
saveRDS(doc_df,file = doc_file_name)
